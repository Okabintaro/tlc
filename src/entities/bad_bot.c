...
static void load(void) {
	image_t *sheet = image("assets/sprites/badbot.qoi");

	// clang-format off
	anim_idle  = anim_def(sheet, vec2i(16, 16), 0.5, {1,2});
	anim_crawl = anim_def(sheet, vec2i(16, 16), 0.1, {0,1});
	anim_jump  = anim_def(sheet, vec2i(16, 16), 0.2, {2,3,4});
	anim_hit   = anim_def(sheet, vec2i(16, 16), 0.1, {7});
	// clang-format on

	image_t *gib_sheet = image("assets/sprites/badbot_gibs.qoi");
	anim_gib = anim_def(gib_sheet, vec2i(5, 5), 10, {0, 1, 2});
...
static void update(entity_t *self) {
	...
	if (!self->bad_bot.seen_player) {
		if (player_dist.x < 64 && player_dist.y < 20) {
			self->bad_bot.seen_player = true;
		}
	} else if (self->on_ground && self->anim.def != anim_hit) {
		self->bad_bot.jump_timer += engine.tick;
		// Init jump
		if (self->anim.def != anim_jump && self->bad_bot.jump_timer > 0.5 &&
		    ((player_dist.x < 40 && player_dist.y > 20) || // near player
		     map_tile_at_px(engine.collision_map, vec2_add(self->pos, vec2(self->size.x * player_dir, self->size.y))) ==
		         1 // or blocked by wall
		     )) {
		...
		// Jump
			...
			self->bad_bot.in_jump = true;
		}

		// Crawl
		else if (self->anim.def != anim_jump && self->bad_bot.jump_timer > 0.2) {
	...
	if (self->bad_bot.in_jump && self->vel.x == 0 && self->anim.def != anim_hit) {
	...
	if (self->on_ground && !was_on_ground && self->anim.def != anim_hit) {
		self->bad_bot.in_jump = false;
		self->bad_bot.jump_timer = 0;
...
entity_vtab_t entity_vtab_bad_bot = {
    .load = load,
    .init = init,
    .update = update,
    .damage = damage,
